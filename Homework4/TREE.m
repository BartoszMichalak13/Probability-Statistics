...
K=10;          %small K <=> I could not find a way to optimize code
n=100;
...
while n<=100
    ...
    for c=1:n
        ...
        elseif c<=usednumbers+parents %here we fill last full row
           ...
           %3 cases:
           %1: we have some double children
           %2: we have #children<=one child per parent
            if doublechildren>0 %case one
                A(c,floor(c/2))=1;
                A(c,2*c)=1;
                A(c,(2*c)+1)=1;

                doublechildren = doublechildren - 1;
            else    %case 2
                A(c,floor(c/2))=1;
                if d==0
                    d=c;
